# -*- coding: utf-8 -*-
"""
Created on Sat Jan 21 19:34:18 2023
...
# scaler = min_max_scaler.fit(norm_data.reshape(-1, norm_data.shape[1]))
# # mise à l'échelle des données d'apprentissage et de test

# norm_data = scaler.transform(data.values[:].reshape(-1, norm_data.shape[1]))
...
X, Y = create_window(np.array(data), 0, None, WINDOW_SIZE)
...
def plot_history(history):
    ...
    plt.axis([max(max(loss),max(val_loss))])
